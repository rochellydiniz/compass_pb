...
def lambda_handler(event, context):
    coleta_filmes_genero(max_paginas=500, max_por_arquivo=100)
    return {
        'statusCode': 200,
        'body': json.dumps('Coleta de séries finalizada e arquivos enviados para o bucket!')
    }


# Função para coletar series de um gênero específico
def coleta_filmes_genero(max_paginas=500, max_por_arquivo=100):
    series = []
    total_paginas_disponiveis = 1
    
    for pagina in range(1, max_paginas + 1):
       url = f"{url_base}/discover/movie?api_key={api_key}&include_adult=false&include_null_first_air_dates=false&with_genres={generos}&language={idioma}&page={pagina}"
